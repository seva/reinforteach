...
- [x] `tests/plugin/live_wiring.test.ts`
  - plugin inactive (no hooks registered, no scheduler) when no agent has `adaptive_learning` config
  - `getSessions` maps session store entries to domain `SessionEntry`; skips subagent sessions
  - `spawnAgent` calls `api.runtime.subagent.run` with `agent:main:*` session key; returns last assistant message
  - `spawnOracle` calls `api.runtime.subagent.run` with `agent:{oracleId}:*` session key
  - confirmation response (pending `from`) short-circuits pipeline; does not invoke `handleFeedbackEvent`
  - scheduler started on `register`
- [x] `src/plugin/feedback_capture.ts` — wire default export: call `createPlugin` with real OpenClaw API implementations — `getSessions` via OpenClaw sessions API, `spawnAgent`/`spawnOracle` via subagent API, `sendMessage`/`awaitResponse` via channel API, `appendToBuffer` via filesystem, `startScheduler` wired to `spawnTrainAndDeploy`; load `adaptive_learning` config from agent config file — Refs #9
